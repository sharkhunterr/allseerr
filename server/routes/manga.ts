...
mangaRoutes.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const filteredAdult = cfg.hideAdult ? raw.filter((r) => !r.isAdult) : raw;
...
mangaRoutes.get(
  ...
  async (req, res) => {
    ...
    if (isNaN(id)) {
      return res
        .status(400)
        .json({ status: 400, message: 'Invalid staff id.' });
    }
...
mangaRoutes.get(
  ...
  async (req, res) => {
    ...
    if (isNaN(id)) {
      return res
        .status(400)
        .json({ status: 400, message: 'Invalid manga id.' });
    }
    ...
    try {
      ...
      return res.status(200).json({
        ...
        authorPhotoUrl:
          primaryStaff?.image?.large ??
          primaryStaff?.image?.medium ??
          undefined,
...
mangaRoutes.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      logger.info(
        `Manga request created: ${body.title} (anilist:${body.anilistId})`,
        {
          label: 'manga',
