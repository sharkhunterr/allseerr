...
mangaSettingsRoutes.put('/metadata-providers', async (req, res) => {
  try {
    const settings = getSettings();
    const body = req.body as Partial<typeof settings.manga.metadataProviders>;
    ...
    settings.manga = {
      ...
      metadataProviders: {
        ...
        ...(body.primarySource === 'anilist'
          ...
          : {}),
        ...(typeof body.anilist === 'boolean' ? { anilist: body.anilist } : {}),
        ...
        ...(typeof body.preferredLanguage === 'string'
          ...
          : {}),
        ...(body.languagePolicy === 'prefer' || body.languagePolicy === 'strict'
...
mangaSettingsRoutes.put('/suwayomi', async (req, res) => {
  try {
    ...
    await settings.save();
    return res.status(200).json({ ...settings.manga.suwayomi, password: '' });
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e);
