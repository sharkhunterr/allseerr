...
interface TestResponseWithMetadata {
...
const messages = defineMessages('components.Settings.BookshelfModal', {
  createbookshelf: 'Add New Bookshelf Server',
  ...
  toastBookshelfTestSuccess: 'Bookshelf connection established successfully!',
  toastBookshelfTestFailure: 'Failed to connect to Bookshelf.',
  add: 'Add Server',
  ...
  mediaTypeHelp:
    'Choose whether this Bookshelf instance handles books or audiobooks. Only one default per type is allowed.',
  mediaTypeBook: 'Books',
  ...
  apiKeyHelp: 'Find it in Bookshelf: Settings > General > Security > API Key',
  baseUrlHelp:
    'If you set a URL Base in Bookshelf (Settings > General > Host), enter it here (e.g. /bookshelf). Leave blank otherwise.',
  externalUrlHelp:
    'For clickable links on media pages when the hostname is not reachable from outside your network.',
  syncEnabledHelp:
    'Scan Bookshelf for existing books so users cannot request content already available.',
  enableSearchHelp:
...
const BookshelfModal = ({
  ...
  const [isValidated, setIsValidated] = useState(bookshelf ? true : false);
  const [isTesting, setIsTesting] = useState(false);
  const [testResponse, setTestResponse] = useState<TestResponseWithMetadata>({
    profiles: [],
    rootFolders: [],
    tags: [],
    metadataProfiles: [],
  ...
  const BookshelfSettingsSchema = Yup.object().shape({
  ...
  const testConnection = useCallback(
    async ({
      ...
      setIsTesting(true);
      try {
        const response = await axios.post<TestResponseWithMetadata>(
          '/api/v1/settings/bookshelf/test',
          {
            hostname,
            apiKey,
            port: Number(port),
        ...
        if (initialLoad.current) {
          addToast(intl.formatMessage(messages.toastBookshelfTestSuccess), {
            appearance: 'success',
            autoDismiss: true,
          });
        }
        ...
        if (initialLoad.current) {
          addToast(intl.formatMessage(messages.toastBookshelfTestFailure), {
            appearance: 'error',
            autoDismiss: true,
          });
        }
  ...
  useEffect(() => {
    if (bookshelf) {
      testConnection({
        apiKey: bookshelf.apiKey,
        hostname: bookshelf.hostname,
        port: bookshelf.port,
        baseUrl: bookshelf.baseUrl,
        useSsl: bookshelf.useSsl,
      });
    }
  }, [bookshelf, testConnection]);
    ...
    >
      <Formik
        initialValues={{
          name: bookshelf?.name,
          hostname: bookshelf?.hostname,
          port: bookshelf?.port ?? 8787, // Bookshelf default port (Readarr was 8787)
          ssl: bookshelf?.useSsl ?? false,
          apiKey: bookshelf?.apiKey,
          baseUrl: bookshelf?.baseUrl,
          activeProfileId: bookshelf?.activeProfileId,
          metadataProfileId: bookshelf?.metadataProfileId ?? 1,
          rootFolder: bookshelf?.activeDirectory,
          mediaType: bookshelf?.mediaType ?? defaultMediaType,
          tags: bookshelf?.tags ?? [],
          isDefault: bookshelf?.isDefault ?? false,
          externalUrl: bookshelf?.externalUrl,
          syncEnabled: bookshelf?.syncEnabled ?? false,
          enableSearch: !bookshelf?.preventSearch,
          tagRequests: bookshelf?.tagRequests ?? false,
        }}
        validationSchema={BookshelfSettingsSchema}
        onSubmit={async (values) => {
          try {
            ...
            const submission = {
              ...
              metadataProfileId: Number(values.metadataProfileId) || 1,
            ...
            if (!bookshelf) {
              await axios.post('/api/v1/settings/bookshelf', submission);
            } else {
              await axios.put(
                `/api/v1/settings/bookshelf/${bookshelf.id}`,
                submission
              );
            }
        ...
        {({
          ...
          return (
            <Modal
              ...
              title={
                !bookshelf
                  ? intl.formatMessage(messages.createbookshelf)
                  : intl.formatMessage(messages.editbookshelf)
              }
            >
              <div className="mb-6">
                ...
                <div className="form-row">
                  ...
                  <div className="form-input-area">
                    ...
                    {errors.mediaType &&
                      touched.mediaType &&
                      typeof errors.mediaType === 'string' && (
                        <div className="error">{errors.mediaType}</div>
                      )}
                  </div>
                </div>
                <div className="form-row">
                  ...
                  <div className="form-input-area">
                    <div className="form-input-field">
                      <Field
                        ...
                        disabled={!isValidated || isTesting}
                      >
                        <option value="1">Standard (default)</option>
                        {(testResponse.metadataProfiles ?? []).map((p) => (
                          <option key={`meta-profile-${p.id}`} value={p.id}>
                            {p.name}
                          </option>
                        ))}
                      </Field>
