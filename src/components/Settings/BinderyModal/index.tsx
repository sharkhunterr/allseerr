...
const messages = defineMessages('components.Settings.BinderyModal', {
  createbindery: 'Add New Bindery Server',
  editbindery: 'Edit Bindery Server',
  ...
  toastBinderyTestSuccess: 'Bindery connection established successfully!',
  toastBinderyTestFailure: 'Failed to connect to Bindery.',
  add: 'Add Server',
  ...
  mediaTypeHelp:
    'Choose whether this Bindery instance handles books or audiobooks. Only one default per type is allowed.',
  mediaTypeBook: 'Books',
  ...
  apiKeyHelp: 'Find it in Bindery: Settings > General > Security > API Key',
  baseUrlHelp:
    'If you set a URL Base in Bindery (Settings > General > Host), enter it here (e.g. /bindery). Leave blank otherwise.',
  externalUrlHelp:
    'For clickable links on media pages when the hostname is not reachable from outside your network.',
  syncEnabledHelp:
    'Scan Bindery for existing books so users cannot request content already available.',
  enableSearchHelp:
    'Automatically trigger a search in Bindery when a request is approved.',
});

interface BinderyModalProps {
...
const BinderyModal = ({
  ...
  const [isValidated, setIsValidated] = useState(bindery ? true : false);
  ...
  const BinderySettingsSchema = Yup.object().shape({
    ...
    async ({
      ...
      try {
        ...
        if (initialLoad.current) {
          addToast(intl.formatMessage(messages.toastBinderyTestSuccess), {
            appearance: 'success',
            autoDismiss: true,
          });
        }
        ...
        if (initialLoad.current) {
          addToast(intl.formatMessage(messages.toastBinderyTestFailure), {
            appearance: 'error',
            autoDismiss: true,
          });
        }
  ...
  useEffect(() => {
    if (bindery) {
      testConnection({
        apiKey: bindery.apiKey,
        hostname: bindery.hostname,
        port: bindery.port,
        baseUrl: bindery.baseUrl,
        useSsl: bindery.useSsl,
      });
    }
  }, [bindery, testConnection]);
    ...
    >
      <Formik
        initialValues={{
          name: bindery?.name,
          hostname: bindery?.hostname,
          port: bindery?.port ?? 8787,
          ssl: bindery?.useSsl ?? false,
          apiKey: bindery?.apiKey,
          baseUrl: bindery?.baseUrl,
          activeProfileId: bindery?.activeProfileId,
          rootFolder: bindery?.activeDirectory,
          mediaType: bindery?.mediaType ?? defaultMediaType,
          tags: bindery?.tags ?? [],
          isDefault: bindery?.isDefault ?? false,
          externalUrl: bindery?.externalUrl,
          syncEnabled: bindery?.syncEnabled ?? false,
          enableSearch: !bindery?.preventSearch,
          tagRequests: bindery?.tagRequests ?? false,
        }}
        validationSchema={BinderySettingsSchema}
        onSubmit={async (values) => {
          try {
            ...
            if (!bindery) {
              await axios.post('/api/v1/settings/bindery', submission);
            } else {
              await axios.put(
                `/api/v1/settings/bindery/${bindery.id}`,
                submission
              );
            }
        ...
        {({
          ...
          return (
            <Modal
              ...
              title={
                !bindery
                  ? intl.formatMessage(messages.createbindery)
                  : intl.formatMessage(messages.editbindery)
              }
            >
              <div className="mb-6">
